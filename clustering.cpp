...
struct DisjointSets {
    ...
    DisjointSets(int size) : size(size), sets(size), count(size) {
...
int main() {
    ...
    vector<Edge> mst;
    double result = 0.0;
    while (!edges.empty() && mst.size() < n - 1 && clusters != uf.count) {
        Edge e = edges.top();
        edges.pop();
        int v = e.id_two;
        int w = e.id_one;
    ...
    while (uf.getParent(edges.top().id_one) == uf.getParent(edges.top().id_two)) {
        edges.pop();
    }

    std::cout << std::setprecision(10) << edges.top().weight << std::endl;
}
