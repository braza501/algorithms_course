...
public class Graph<TDataValue extends Comparable, TWeight extends Comparable> {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String[] vertices = bufReader.readLine().split(" ");
        int i = 0;
        for (String str : vertices) {
            map.put(str, i++);
            graph.addVertex(str);
        }
        vertices = bufReader.readLine().split(" ");

        for (int j = 0; j < vertices.length - 2; j += 3) {
            int vertex1 = map.get(vertices[j]);
            int vertex2 = map.get(vertices[j + 1]);
            graph.addEdge(graph.vertices.get(vertex1), graph.vertices.get(vertex2), Integer.valueOf(vertices[j + 2]));
        }
        graph.serialize("ololo.txt");
    }
